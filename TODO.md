...
- [ ] Inline-воспроизведение видео (§4.3)
- [ ] Альбомы / группы медиа (`groupedId`, `messages` с общим альбомом)
- [ ] Сжатие vs «как файл» при отправке фото/видео
- [ ] Видеосообщения («кружочки»): `inputMessageVideoNote`, `messageVideoNote`
- [ ] Просмотрщик медиа на весь экран: зум, свайп между фото, сохранить

#### Аудио и файлы
- [x] Отправка документов (`file_picker`)
- [ ] Голосовые сообщения: запись, `inputMessageVoice`, воспроизведение с waveform
- [ ] Аудиофайлы / музыка: `messageAudio`, встроенный плеер, метаданные (исполнитель, обложка)
- [ ] Прогресс загрузки/скачивания (`updateFile`, `uploadFile`, `cancelUploadFile`)

#### Кэш и автозагрузка
- [ ] Кэширование медиа на диск (§4.3)
- [ ] Настройки автозагрузки по типу сети (`autoDownloadSettings` — Wi‑Fi / mobile / roaming)
- [ ] Очистка кэша (`optimizeStorage`, выборочное удаление)
