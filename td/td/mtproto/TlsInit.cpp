...
class TlsHello {
 public:
  ...
  static const TlsHello &get_for_profile(dpi_bypass::TlsProfile profile) {
    ...
    static const TlsHello firefox = [] {
      TlsHello res;
      res.ops_ = build_firefox_ops();
      return res;
    }();
    static const TlsHello yandex = [] {
      TlsHello res;
      res.ops_ = build_yandex_ops();
      return res;
    }();
    static const TlsHello safari = [] {
      TlsHello res;
      res.ops_ = build_safari_ops();
      res.grease_size_ = 7;
      return res;
    }();
 ...
 private:
  // DPI_BYPASS: Chrome-подобный профиль (базовый для Linux/Windows).
  static std::vector<Op> build_chrome_ops() {
    return {
        Op::str("\x16\x03\x01"),
        Op::begin_scope(),
        Op::str("\x01\x00"),
  ...
  // DPI_BYPASS: Firefox — другой порядок cipher suites.
  static std::vector<Op> build_firefox_ops() {
    auto ops = build_chrome_ops();
    for (auto &op : ops) {
      if (op.type == Op::Type::String && op.data.size() > 4 && op.data[0] == '\x13') {
        op.data =
            "\x13\x03\x13\x01\x13\x02\xc0\x2c\xc0\x30\xc0\x2b\xc0\x2f\xcc\xa9\xcc\xa8\xc0\x0a\xc0\x09\xc0\x14\xc0\x13"
            "\x00\x9d\x00\x9c\x00\x35\x00\x2f\x01\x00";
        break;
      }
    }
    return ops;
  ...
  static std::vector<Op> build_yandex_ops() {
    return {
        Op::str("\x16\x03\x01"),
        Op::begin_scope(),
        Op::str("\x01\x00"),
  ...
  // DPI_BYPASS: Safari — профиль macOS/iOS.
  static std::vector<Op> build_safari_ops() {
    return {
        Op::str("\x16\x03\x01"),
        Op::begin_scope(),
        Op::str("\x01\x00"),
...
void TlsInit::send_hello() {
  ...
  // DPI_BYPASS: фрагментация ClientHello на несколько TCP-сегментов.
  auto fragments = dpi_bypass::fragment_client_hello(hello);
  for (const auto &fragment : fragments) {
    fd_.output_buffer().append(fragment);
  }
