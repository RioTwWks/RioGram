...
void ObfuscatedTransport::init(ChainBufferReader *input, ChainBufferWriter *output) {
  ...
  if (secret_.emulate_tls()) {
    // DPI_BYPASS: динамический размер TLS-записи для каждого соединения.
    max_tls_packet_length_ =
        static_cast<int32>(dpi_bypass::pick_random_record_size(static_cast<size_t>(MAX_TLS_PACKET_LENGTH)));
