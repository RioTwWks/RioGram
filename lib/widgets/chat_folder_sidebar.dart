  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(width: width, color: tg.chatListBackground, child: SafeArea(child: Column(children: [
      Expanded(child: ListView(padding: const EdgeInsets.symmetric(vertical: 8), children: [
        _SidebarIconButton(icon: Icons.chat_bubble_outline, tooltip: 'Все чаты', selected: activeList is ChatListMain, onTap: () => onSelected(const ChatListMain())),
        ...
        _SidebarIconButton(icon: Icons.archive_outlined, tooltip: 'Архив', selected: activeList is ChatListArchive, onTap: () => onSelected(const ChatListArchive())),
        if (hasSavedMessages && onSavedMessages != null) _SidebarIconButton(icon: Icons.bookmark_outline, tooltip: 'Избранное', selected: false, onTap: onSavedMessages!),
      ])),
      _SidebarIconButton(icon: Icons.settings_outlined, tooltip: 'Настройки', selected: false, onTap: onSettings),
      const SizedBox(height: 8),
    ])));
  }
  static IconData _folderIcon(String? iconName) => switch (iconName) { 'Briefcase' => Icons.work_outline, 'Heart' => Icons.favorite_outline, 'Book' => Icons.menu_book_outlined, 'Star' => Icons.star_outline, 'Personal' => Icons.person_outline, 'Groups' => Icons.group_outlined, 'Channels' => Icons.campaign_outlined, 'Bots' => Icons.smart_toy_outlined, _ => Icons.folder_outlined };
}
